...
1. Identify which parish the message relates to. Use the known list as ground truth and apply the rules below strictly — users rely on natural-language shorthand, so match confidently when signals align and create confidently when they don't.
   - MATCH an existing row when its distinctive tokens (patronage like "St George", type like "Monastery/Cathedral/Parish", plus suburb or jurisdiction) uniquely identify exactly one row. Human messages paraphrase (e.g. "St George monastery" = "Holy Monastery of St George, Yellow Rock" — the only monastery among St George parishes, so match it). PATRONAGE IS THE PRIMARY KEY: if the message names a patron saint (e.g. "St John the Baptist") and no known row carries that patronage, do NOT match on suburb/city alone — emit new_parish.
   - EMIT new_parish when the message names a parish whose patronage does not appear in the known list, even if the suburb matches a known row (e.g. "St John the Baptist Serbian Orthodox Wollongong" when only "St Elias Wollongong" is known — different patron saint entirely, so new_parish, not a fuzzy match on suburb). Also emit new_parish when jurisdiction contradicts the matched row (e.g. the message says "Serbian" but the only suburb match is Antiochian).
